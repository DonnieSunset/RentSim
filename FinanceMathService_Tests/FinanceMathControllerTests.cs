...
using FinanceMathService.Controllers;

namespace FinanceMathService_Tests
{
    public class FinanceMath_uTests
    {
        [TestCase(2500, 1, 2000, 2000)]
        [TestCase(2500, 0.9, 2000, 1944.44444)]
        [TestCase(2500, 0.8, 2000, 1875)]
        [TestCase(2500, 0.7, 2000, 1785.71428)]
        [TestCase(2500, 0.6, 2000, 1666.66666)]
        [TestCase(2500, 0.5, 2000, 1500)]
        [TestCase(2500, 0.4, 2000, 1250)]
        [TestCase(2500, 0.3, 2000, 833.33333)]
        [TestCase(2500, 0.2, 2000, 0)]
        [TestCase(2500, 0.1, 2000, 0)]
        [TestCase(2500, 0, 2000, 0)]
        ...
        {
            double lowRiskAMount = FinanceMathController.NonRiskAssetsNeededInCaseOfRiskAssetCrash(totalAmount, stocksCrashFactor, totalAmountMinNeededAfterCrash);
            ...
            Assert.That(lowRiskAMount + (highRiskAmount * (1 - stocksCrashFactor)), Is.GreaterThanOrEqualTo(totalAmountMinNeededAfterCrash).Within(0.001));
        ...
        {
            Assert.That(() => FinanceMathController.NonRiskAssetsNeededInCaseOfRiskAssetCrash(totalAmount, stocksCrashFactor, totalAmountMinNeededAfterCrash), Throws.ArgumentException);
        }
        ...
        public void Test_RateByNumericalSparkassenformel(double betrag1, double zins1, double betrag2, double zins2, double betrag3, double zins3, double endBetrag, int anzahlJahre)
        {
            ...
            double rate = FinanceMathController.RateByNumericalSparkassenformel(

...
namespace FinanceMathService_Tests
{
    ...
    public class FinanceMathTests
    {
        ...
        public void StartCapitalByNumericalSparkassenformel_DefaultInput_SingleAssetsResultToZero()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(proto.Protocol.Last().cash.YearEnd, Is.EqualTo(0).Within(Tolerance));
                Assert.That(proto.Protocol.Last().stocks.YearEnd, Is.EqualTo(0).Within(Tolerance));
                Assert.That(proto.Protocol.Last().metals.YearEnd, Is.EqualTo(0).Within(Tolerance));
            });
        }
